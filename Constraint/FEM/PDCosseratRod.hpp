...
public:
                                  ...
                                  const uint e)
    {
        ...
        p[a].getCenter () += cRod.wq(a) * w0 * gamma;
        p[z].getCenter () -= cRod.wq(z) * w1 * gamma;
...
//        //  COMPUTE STRETCHING AND SHEARING
        ...
        gamma     /= (w1 + w0) / cRod.length(a)+ cRod.wq(a) * static_cast<SReal>(4.0)*cRod.length(a);
        ...
        m_dq[a].coeffs() += (cRod.wq(a) * cRod.length(a) * cRod.wbt(a)) * u0.coeffs ();
        ...
        for (uint i = 0; i < 3; i++)
            omega.coeffs()[i] *= bending_twisting[i] / (cRod.wq(a) + cRod.wq(z));
        omega.w() = 0.0;    //discrete Darboux vector does not have vanishing scalar part
        m_dq[a].coeffs () += (cRod.wq(a) * cRod.wbt(a)) * (u[z] * omega).coeffs ();
        m_dq[z].coeffs () -= (cRod.wbt(z) * cRod.wq (z)) * (u[a] * omega).coeffs ();
    }
